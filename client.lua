    ...
    for k,v in pairs(Config.alerts) do 
        RegisterCommand(v.command, function(source, args, rawCommand)
            if not cooldown then 
                ...
                TriggerServerEvent("skrievo_alert:sendalert",GetPlayers(),v.jobs,v.msg,v.blip,v.isdoctor)
                cooldown = true
            end
        end, false)
    end
...
RegisterNetEvent("skrievo_alert:createblip")
AddEventHandler("skrievo_alert:createblip", function(blip,dx,id)
    isalerted = true 
    doctor = dx
    serverid = id
...
RegisterNetEvent("skrievo_alert:recresp")
AddEventHandler("skrievo_alert:recresp", function()
   if isalerted then 
        isalerted = false 
   end
   isalerted2 = true 
...
RegisterNetEvent("skrievo_alert:cantalert")
AddEventHandler("skrievo_alert:cantalert", function()
   if isalerted then 
        isalerted = false 
   end
   if isalerted2 then 
...
RegisterCommand(Config.medicresponse, function(source, args, rawCommand)
    if doctor then 
        doctor = false 
        TriggerServerEvent("skrievo_alert:inform",serverid)
    end
end, false)
